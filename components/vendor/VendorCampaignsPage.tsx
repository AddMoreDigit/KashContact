...
import { getAllCampaigns, updateCampaignStatusOnly, type Campaign } from '../../utils/campaignStorage';
...
export function VendorCampaignsPage({ onNavigate, onSelectCampaign }: VendorCampaignsPageProps) {
  ...
  const handleAcceptCampaign = (campaignId: string, campaignName: string) => {
    // Update campaign status in storage
    updateCampaignStatusOnly(campaignId, 'accepted');
  ...
  const handleDeclineCampaign = (campaignId: string, campaignName: string) => {
    // Update campaign status in storage
    updateCampaignStatusOnly(campaignId, 'declined');
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                {campaign.creatorEmail === campaign.organizerEmail && (
                                  <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded">Creator</span>
                                )}
                              </div>
                              <p className="text-xs text-gray-500">{formattedDates}</p>
                              {campaign.adminEmails && campaign.adminEmails.length > 1 && (
                                <p className="text-xs text-blue-600 mt-1">
                                  {campaign.adminEmails.length} admin{campaign.adminEmails.length > 1 ? 's' : ''}
                                </p>
                              )}
                            </div>
                          </div>
