...
export function calculateProfileCompletion(data: ProfileCompletionData): ProfileCompletionResult {
  ...
  // Bank Details (25%) - Based on having complete banking information
  const hasBankAccountHolder = !!data.bankDetails?.accountHolder && data.bankDetails.accountHolder.trim() !== '';
  const hasBankAccountNumber = !!data.bankDetails?.accountNumber && data.bankDetails.accountNumber.trim() !== '';
  const hasBankName = !!data.bankDetails?.bankName && data.bankDetails.bankName.trim() !== '';
  
  const bankDetailsFields = [hasBankAccountHolder, hasBankAccountNumber, hasBankName];
