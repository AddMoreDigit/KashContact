...
import { SlidersHorizontal, ChevronLeft, ChevronRight, TrendingUp, TrendingDown, DollarSign, Package, Users } from "lucide-react";
...
export default function App() {
  ...
  // UI state
  const [date, setDate] = useState<Date | undefined>(new Date(2025, 2, 1));
  ...
  const handleSearch = (query: string) => {
    ...
    // Search is handled in filtered data
  };
                  ...
                  filteredCampaigns.map((campaign) => (
                    <div key={campaign.id} className="flex gap-4 group">
                      ...
                      <div className="flex-1">
                        <CampaignCard {...campaign} onCancel={() => handleCampaignCancel(campaign.id)} />
                      </div>
                    </div>
                  ))
                ) : (
