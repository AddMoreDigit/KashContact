...
export function storedToAppCampaign(stored: StoredCampaign): AppCampaign {
  ...
  // Determine status - campaign admins can manage, regular members can only contribute
  const userProfile = JSON.parse(localStorage.getItem('userProfile') || '{}');
  const userEmail = userProfile.email || '';
  // Check if user is a campaign admin (creator or co-admin)
  const isAdmin = stored.adminEmails?.includes(userEmail) || false;
  const status: 'contribute' | 'manage' = isAdmin ? 'manage' : 'contribute';
